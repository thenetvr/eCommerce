...
export default function Footer() {
  const socialMedia = {
    ...
    "https://www.instagram.com/thenetvr": InstagramIcon,
    "https://twitter.com/thenetvr": TwitterIcon,
    "https://discord.com/invite/DpNy4Ur": DiscordIcon,
    "https://www.youtube.com/channel/UCWOD7IdEz0z6S-K3Sf6B86Q": YoutubeIcon,
  };
  const Icons = () => {
    const formatIcons = [];
    let i = 0;
    for (let media in socialMedia) {
      ...
      if (i % 2 !== 0 || i === Object.keys(socialMedia).length - 1) {
        formatIcons.push(
          <Link href={media}>
            <Image
              className="border h-[40px] w-[40px] mx-[20px] bg-indigo-500 rounded-full"
        ...
        formatIcons.push(
          <Link href={media}>
            <Image
              className="h-[40px] w-[40px] mx-[20px] bg-indigo-500 rounded-full"
              src={socialMedia[media]}
              alt={"can't display"}
            />{" "}
          </Link>
  ...
  return (
    <div className="flex flex-col justify-center bg-black w-full h-[480px]">
      <div className="flex justify-evenly text-white border-b border-solid border-white items-start">
        ...
        <div className="flex flex-col w-[450px] h-72 bg-[#343434] rounded-[30px] mb-4 [&>*]:mb-4 [&>*]:ml-10">
          ...
          <div className="flex w-fit">
            <input
              className="rounded-l-md h-14 bg-white w-fit"
              type={"text"}
              placeholder="Email address"
            />
            <button className="rounded-r-sm bg-blue-500 h-full w-14">
              {" "}
      ...
      <div className="text-white w-full mt-10 text-center">
        ...
        <div className="inline-flex float-right">{Icons()}</div>
      </div>
    </div>
  );
}
